...
def translateString_3way(s):
    trans_dict = {'A':'10','B':'20','1':'11','0':'00','2':'22'}
    translated_string=''.join([trans_dict[x] for x in s])
    return(translated_string)
...
#def generateAdmixStrings_branch_3way(t,p,n=2):
#    '''
#    generate shuffled admixstrings for one branch given one value of p. default number of shuffles for each string is 2. No subadmixture
#    '''
##{{{ 
...
#    else:
#        num_founder_branches = 2**(t-1)
#        base_string = '1'*(num_founder_branches//2)+ '2'*(num_founder_branches//2)
#
#        prop_admixed_branches = p*2
#        allowed_prop_admixed_branches = [x/num_founder_branches for x in range(num_founder_branches+1)]
#	    
...
#            l=list(base_string)
#            random.shuffle(l)
#            base_string_shuf=''.join(l)
#
#            replace_inds = random.sample(range(0, len(base_string_shuf)), num_admixed_branches)
#
#            rem_shufs-=1
#            string=base_string_shuf
#
#            for i in replace_inds: #Replace sampled founder branches with 'A1' or 'A2' for admixed
#                if i < len(base_string)//2:
#                    string=string[:i] +'A' + string[i+1:]
#                else:
#                    string=string[:i] +'B' + string[i+1:]
#
#            final_string=translateString_3way(string) #Translate branches string to admixstring
#            strings.append(final_string)
##}}}
#    return(list(set(strings)))


def generateAdmixStrings_branch_3way(t,p0,p2,n=2):
    '''
...
#{{{ 
    ...
    base_strings=generateAdmixStrings_branch(t,p0,n)

    repl_strings=[]
    for i in range(n):
        
        s=base_strings[i] if i < len(base_strings) else s #Use the last string if generateAdmixStrings_branch runs out of strings

        inds_het = [pos-1 for pos, char in enumerate(s) if char == '0']
        inds_repl=random.sample(inds_het,len(inds_het)//2)
        inds_0= [pos for pos, char in enumerate(s) if char == '0']

        inds_other=[i for i in range(len(s)) if i not in inds_repl+inds_0 ]


        l=list(s)
        l_repl=['2' if idx in inds_repl else e for idx,e in enumerate(l)]
        s_repl=''.join(l_repl)

        n_remain = int(p2*len(s) - len(inds_repl)) 
        inds_repl=random.sample(inds_other,n_remain)
        
        l=list(s_repl)
        l_repl=['2' if idx in inds_repl else e for idx,e in enumerate(l)]
        s_repl=''.join(l_repl)

        repl_strings.append(s_repl)
#}}}
    return(repl_strings)
...
def modify_subadmixStrings(s):
    '''
    modifies sub admixed strings by increasing the proportion of branches that are '10'. Spits out all possible variants
    '''
    longestMatch=False #longestMatch is the longest match that can be replaced by '10's in while keeping the subadmixture structure
    n=2 
    while not longestMatch:
        match_pattern=''.join([char*n for char in '10'])
        if match pattern in s:
            num_matches=s.count(match_pattern)
            for match in num_matches:
                repl_string='10'*n
                s.replace(match_pattern,repl_string,1)
